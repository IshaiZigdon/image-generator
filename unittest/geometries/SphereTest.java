...
public class SphereTest {
    ...
    public void testFindGeoIntersectionsHelper() {
        ...
        var exp = List.of(new GeoPoint(sphere, gp1), new GeoPoint(sphere, gp2));
        ...
        assertNull(sphere.findGeoIntersections((new Ray(p01, v110))), "TC01: Ray's line out of sphere");
        // TC02: Ray starts before and crosses the sphere (2 points)
        final var result02 = sphere.findGeoIntersections(new Ray(p01, v310)).stream().sorted(Comparator.comparingDouble(p -> p.point.distance(p01))).toList();
        assertEquals(2, result02.size(), "TC02: Wrong number of points");
        assertEquals(exp, result02, "TC02: Ray crosses sphere");
        // TC03: Ray starts inside the sphere (1 point)
        final var result03 = sphere.findGeoIntersections(new Ray(p02, v310));
        ...
        assertNull(sphere.findGeoIntersections(new Ray(p03, v310)), "TC04: Ray's line out of sphere");

        // =============== Boundary Values Tests ==================

        // **** Group: Ray's line crosses the sphere (but not the center)
        // TC10: Ray starts at sphere and goes inside (1 point)
        final var result11 = sphere.findGeoIntersections(new Ray(gp1, v310));
        ...
        assertNull(sphere.findGeoIntersections(new Ray(gp2, v310)), "TC11: Ray's line out of sphere");
        // **** Group: Ray's line goes through the center
        final Vector v300 = new Vector(3, 0, 0);
        final Point p200 = new Point(2, 0, 0);
        // TC12: Ray starts before the sphere (2 points)
        final var result13 = sphere.findGeoIntersections(new Ray(p01, v300)).stream().sorted(Comparator.comparingDouble(p -> p.point.distance(p01))).toList();
        ...
        final var result14 = sphere.findGeoIntersections(new Ray(Point.ZERO, v300));
        ...
        final var result15 = sphere.findGeoIntersections(new Ray(pHalf, v300));
        ...
        final var result16 = sphere.findGeoIntersections(new Ray(p100, v300));
        ...
        assertNull(sphere.findGeoIntersections(new Ray(p200, v300)), "TC16: Ray's line out of sphere");
        // TC17: Ray starts after sphere (0 points)
        final Point p2Half = new Point(2.5, 0, 0);
        assertNull(sphere.findGeoIntersections(new Ray(p2Half, v300)), "TC17: Ray's line out of sphere");
        // **** Group: Ray's line is tangent to the sphere (all tests 0 points)
        // TC18: Ray starts before the tangent point
        final Point p010 = new Point(0, 1, 0);
        assertNull(sphere.findGeoIntersections(new Ray(p010, v300)), "TC18: Ray's line out of sphere");
        // TC19: Ray starts at the tangent point
        final Point p110 = new Point(1, 1, 0);
        assertNull(sphere.findGeoIntersections(new Ray(p110, v300)), "TC19: Ray's line out of sphere");
        // TC20: Ray starts after the tangent point
        // p03 = new Point(2, 1, 0);
        assertNull(sphere.findGeoIntersections(new Ray(p03, v300)), "TC20: Ray's line out of sphere");
        // **** Group: Special cases
        // TC21: Ray's line is outside, ray is orthogonal to ray start to sphere's center line
        final Point p020 = new Point(0, 2, 0);
        assertNull(sphere.findGeoIntersections(new Ray(p020, v300)), "TC21: Ray's line out of sphere");
        // TC22: Ray's line is inside, ray is orthogonal to ray start to sphere's center line
        final Point p04 = new Point(0.8, 0.6, 0);
        final var result22 = sphere.findGeoIntersections(new Ray(p04, v310));
